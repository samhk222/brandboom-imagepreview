...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $this->file_name_after_upload = $this->options['prepend_file_name'] .$this->sanitize_file_name($file->name);
            $file_path = $this->get_upload_path( $this->file_name_after_upload );
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $sql     = "insert into images (filepath, created_at) values ('{$this->file_name_after_upload}', now())";
                    DB::query($sql);
                    $this->make_thumb($this->options['upload_dir'] ."thumbs/", $file_path, 200);
                    if ($this->options['save_to_session']){
                        $_SESSION['_uploaded_files'][] = $this->file_name_after_upload;
                    }
                }
            } else {
    ...
    public function sanitize_file_name($str, $replace=array(), $delimiter='-', $keep_file_name = false) {
    ...
    public function make_thumb($updir, $img, $id,$MaxWe=100,$MaxHe=150){
